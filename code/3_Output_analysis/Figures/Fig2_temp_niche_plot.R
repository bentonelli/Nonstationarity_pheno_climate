...
d2 <- readRDS("data/mdl_data/ms_data2.rds") %>% select(c("species","cell","mean_tmin_29","mean_prcp_29","mean_swe_29","sd_tmin_c29","sd_prcp_c29","sd_swe_c29"))
...
par(las=1)
par(pty="s")
plot(NULL,xlim=c(-20,20),ylim=c(-3,1.5),xlab="Mean Temp, Arrival", ylab="Arrival Change, Days/ +1 Degree",
...
temp_plot <- ggplot(sp_temp,aes(x=mean_tmin_29,y=days_per_degree,fill=latitude)) +
  ...
  ylab("Days per Degree (\u00B0C)") +
  theme(aspect.ratio=1,
        text = element_text(size = 20,family = "sans"),
        axis.text = element_text(size = 20,colour = "black"),
        legend.position = c(0.76, 0.1),
        legend.key.size = unit(1.75, 'cm'), #change legend key size
        legend.key.height = unit(1.75, 'cm'), #change legend key height
        legend.key.width = unit(1.75, 'cm'), #change legend key width
