...
#sp_ind <- which(d2$species == "Eastern Phoebe")
var_exp_cm_c <- c()
for(each_spec in unique(d2$species)){
  sp_ind <- which(d2$species == each_spec)
  ...
  for (nn in samp_ind){
    ...
    # TMIN
    grep_temp <-  as.numeric(mb1[nn,d2$cl_sp[sp_ind]] * d2$anom_tmin_c29[sp_ind])
    e_temp <- (yy_trues[nn,sp_ind] - ma1[nn,d2$cl_sp[sp_ind]]) - grep_temp
    var_exp_temp <- var(grep_temp)/(var(grep_temp) + var(e_temp))
    c_var_exp_temp <- c(c_var_exp_temp,var_exp_temp)
    
    # SWE
    grep_swe <-  as.numeric(mb2[nn,d2$cl_sp[sp_ind]] * snow_p[sp_ind] * d2$anom_swe_c29[sp_ind])
    e_swe <- (yy_trues[nn,sp_ind] - ma1[nn,d2$cl_sp[sp_ind]]) - grep_swe
    var_exp_swe <- var(grep_swe)/(var(grep_swe) + var(e_swe))
    c_var_exp_swe <- c(c_var_exp_swe,var_exp_swe)
    
    # Precip
    grep_prcp <-  as.numeric(mb3[nn,d2$cl_sp[sp_ind]] * d2$anom_prcp_c29[sp_ind])
    e_prcp <- (yy_trues[nn,sp_ind] - ma1[nn,d2$cl_sp[sp_ind]]) - grep_prcp
    var_exp_prcp <- var(grep_prcp)/(var(grep_prcp) + var(e_prcp))
    c_var_exp_prcp <- c(c_var_exp_prcp,var_exp_prcp)
    
    # ENSO
    grep_enso <-  as.numeric(ph1[nn,d2$cl_sp[sp_ind]] * d2$ENSO[sp_ind])
    e_enso <- (yy_trues[nn,sp_ind] - ma1[nn,d2$cl_sp[sp_ind]]) - grep_enso
    var_exp_enso <- var(grep_enso)/(var(grep_enso) + var(e_enso))
    c_var_exp_enso <- c(c_var_exp_enso,var_exp_enso)
    
    #NAO
    grep_nao <-  as.numeric(ph2[nn,d2$cl_sp[sp_ind]] * d2$NAO[sp_ind])
    e_nao <- (yy_trues[nn,sp_ind] - ma1[nn,d2$cl_sp[sp_ind]]) - grep_nao
    var_exp_nao <- var(grep_nao)/(var(grep_nao) + var(e_nao))
    c_var_exp_nao <- c(c_var_exp_nao,var_exp_nao)
    
    # EVERYTHING
    grep_all <- as.numeric(mb1[nn,d2$cl_sp[sp_ind]] * d2$anom_tmin_c29[sp_ind] + 
                             mb2[nn,d2$cl_sp[sp_ind]] * snow_p[sp_ind] * d2$anom_swe_c29[sp_ind] + 
                             mb3[nn,d2$cl_sp[sp_ind]] * d2$anom_prcp_c29[sp_ind] +
                             ph1[nn,d2$cl_sp[sp_ind]] * d2$ENSO[sp_ind] +
                             ph2[nn,d2$cl_sp[sp_ind]] * d2$NAO[sp_ind])
    e_all <- (yy_trues[nn,sp_ind] - ma1[nn,d2$cl_sp[sp_ind]]) - grep_all
    var_exp_all <- var(grep_all)/(var(grep_all) + var(e_all))
    c_var_exp_all <- c(c_var_exp_all,var_exp_all)
    
    # Variance, explained or otherwise
    var_all <- var(yy_trues[nn,sp_ind] - ma1[nn,d2$cl_sp[sp_ind]])
    c_var_all <- c(c_var_all,var_all)
    
    # Mean arrival across all cells, years
    arr_mean_all <- mean(yy_trues[nn,sp_ind]+arr_mean_cross_sp)
...
var_exp_cm_c$species <- unique(d2$species)
spec_groups <- read.csv("spec_groups.csv")
var_exp_cm_c <- merge(var_exp_cm_c,spec_groups,by = "species")

write.csv(var_exp_cm_c,"data/output/var_exp/var_exp_sp.csv")
